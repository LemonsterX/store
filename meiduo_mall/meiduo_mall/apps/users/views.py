...
class MobileCountView(APIView):
    ...
    def get(self, request, mobile):
        ...
        count = mobile = User.objects.filter(mobile=mobile).count()
        data = {
            'mobile': mobile,
            'count': count
        }
...
class EmailVerifyView(APIView):
    def put(self, request):
        ...
        if user is None:
            return Response({'message': '无效token数据'}, status=status.HTTP_400_BAD_REQUEST)
        # 2. 设置对应用户的邮箱验证标记email_active
...
class BrowseHistoryView(CreateAPIView):
    ...
    def get(self, request):
        ...
        # [b'<sku_id>', b'<sku_id>', ...] 获取用户浏览所有商品id
